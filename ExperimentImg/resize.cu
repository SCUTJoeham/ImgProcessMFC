...
__global__ void Resize(uchar* _src_dev, uchar * _dst_dev, int _src_step, int _dst_step,
	int _src_rows, int _src_cols, int _dst_rows, int _dst_cols)
{
	int i = blockIdx.x;
	int j = blockIdx.y;

	double x = i * (_src_rows*1.0 / _dst_rows);
	double y = j * (_src_cols*1.0 / _dst_cols);
...
extern "C" void Resize_host(const cv::Mat &src, cv::Mat &dst, const cv::Size &dst_s)
{
	...
	Resize << < grid, 1 >> > (src_dev, dst_dev, src_step, dst_step, srcHeight, srcWidth, dst_s.height, dst_s.width);
